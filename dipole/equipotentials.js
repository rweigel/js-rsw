function equipotentials(el) {
    ...
    let p = charges(5); // Location of charges
    let P = implicit(iparams); // Contour

    let board = JXG.JSXGraph.initBoard("jsxbox",bparams);

    for (let i = 0;i < p.length;i++) { // Draw charges
        if (p[i][2] > 0) {
            board.create('point', p[i].slice(0,2),{fillColor:'red',strokeColor:'red',withLabel: false});
        } else {
            board.create('point', p[i].slice(0,2),{fillColor:'green',strokeColor:'green',withLabel: false});
        }
    }

    board.create("curve", P.border, cparams.border); // Draw countour
    board.create("curve", P.interior, cparams.fill); // Draw contour interior
...
function potential(x,y,p) {
    ...
    for (var i = 0; i < p.length;i++) {
        V = V+p[i][2]/Math.sqrt((y-p[i][1])*(y-p[i][1]) + (x-p[i][0])*(x-p[i][0]));
    }
    return V;
}

function implicit(parameters) {
    // From https://jsfiddle.net/tparise/tbuuxhjp/
    ...
    // default settings
    var param = {
        xMin: parameters.xMin || -1,
        xMax: parameters.xMax || 1,
        yMin: parameters.yMin || -1,
        yMax: parameters.yMax || 1,
        xInitialSteps: 1,
        yInitialSteps: 1,
    ...
    implicitFun = function (x,y) {
        var out = parameters.equation(x,y, parameters.sources);
    ...
    return {border: [borderX,borderY],interior: [interiorX,interiorY]};
